...
func GetQuestionE(c *gin.Context) {
	...
	err := db.QueryRow(context.Background(), `select idQa, Exam, Question from QuestionsExam where idQe=$1`, requestBody).Scan(&response.IdQe, &response.Exam, &response.Question)
	if err != nil {
		log.Print("db error")
